...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status" && strCommand != "dumpkey"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot masternode configured in futurocoin.conf\n"
                ...
                "  dumpkey      - Print masternode's public key in HEX\n"
                );
    ...
    if (strCommand == "debug")
    {
        // ToDo: Fix 'debug' command. There is no more information about outpoint.
    ...
    if (strCommand == "status")
    {
        ...
        mnObj.push_back(Pair("pubkey", HexStr(activeMasternode.pubKeyMasternode)));
    ...
    if (strCommand == "dumpkey")
    {
        if (params.size() < 2)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Please specify a private key");

        std::string strKeyMasternode(params[1].get_str());

        CPubKey pubKeyMasternodeNew;
        CKey keyMasternodeNew;

        if (!CMessageSigner::GetKeysFromSecret(strKeyMasternode, keyMasternodeNew, pubKeyMasternodeNew))
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid masternode private key");

        return HexStr(pubKeyMasternodeNew);
    }
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print FuturoCoin address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
