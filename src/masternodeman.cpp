...
struct CompareLastPaidBlock
{
                    ...
                    const std::pair<int, CMasternode*>& t2) const
    {
        ...
        return (t1.first != t2.first)
            ? (t1.first < t2.first)
            : (t1.second->pubKeyMasternode < t2.second->pubKeyMasternode);
    }
};
...
bool CMasternodeMan::Add(CMasternode &mn)
{
    ...
    if (Has(mn.pubKeyMasternode)) {
        return false;
    } else {
        mapMasternodes[mn.pubKeyMasternode] = mn;
...
void CMasternodeMan::AskForMN(CNode* pnode, const CPubKey& pubKey, CConnman& connman)
{
    if (!pnode) return;

    LOCK(cs);

    std::map<CPubKey, std::map<CNetAddr, int64_t> >::iterator it1 = mWeAskedForMasternodeListEntry.find(pubKey);
    ...
    if (it1 != mWeAskedForMasternodeListEntry.end()) {
        ...
        if (it2 != it1->second.end()) {
            ...
            LogPrintf("CMasternodeMan::AskForMN -- Asking same peer %s for missing masternode entry again: %s\n",
                       pnode->addr.ToString(), pubKey.GetID().ToString());
        } else {
    ...
    mWeAskedForMasternodeListEntry[pubKey][pnode->addr] = GetTime() + DSEG_UPDATE_SECONDS;

    connman.PushMessage(pnode, NetMsgType::DSEG, pubKey);
}

bool CMasternodeMan::PoSeBan(const CPubKey& pubKey)
{
    LOCK(cs);
    CMasternode* pmn = Find(pubKey);
    if (!pmn) {
        return false;
    }
    pmn->PoSeBan();
...
void CMasternodeMan::Check()
{
    ...
    LogPrint("masternode", "CMasternodeMan::Check -- cyclic check for %d masternodes\n",
             mapMasternodes.size());
...
void CMasternodeMan::CheckAndRemove(CConnman& connman)
{
...

    {
        ...
        /* Remove spent masternodes, prepare structures and make requests to
         * reassure the state of inactive ones.
         */
        rank_pair_vec_t vecMasternodeRanks;
        // ask for up to MNB_RECOVERY_MAX_ASK_ENTRIES masternode entries at a time
        int nAskForMnbRecovery = MNB_RECOVERY_MAX_ASK_ENTRIES;
        std::map<CPubKey, CMasternode>::iterator it = mapMasternodes.begin();

        while (it != mapMasternodes.end()) {
            CMasternodeBroadcast mnb = CMasternodeBroadcast(it->second);
            uint256 hash = mnb.GetHash();

            /* It is a good place to check whether a masternode is still on
             * the list propagated with 'mnlist' mechanism.
             */
            if (!masternodeListManager.IsMNActive(it->first)) {
                LogPrint("masternode", "CMasternodeMan::CheckAndRemove -- "
                         "remove inactive masternode: %s, addr=%s, size=%i now\n",
                ...
                /* Assume the masternode is valid. */
                bool fAsk = (nAskForMnbRecovery > 0) && masternodeSync.IsSynced() &&
                ...
                if (fAsk) {
                    ...
                    for (int i = 0; setRequested.size() < MNB_RECOVERY_QUORUM_TOTAL &&
                         i < (int)vecMasternodeRanks.size(); i++) {
                        // avoid banning
                        if (mWeAskedForMasternodeListEntry.count(it->first) &&
                            mWeAskedForMasternodeListEntry[it->first].count(vecMasternodeRanks[i].second.addr)) {
                            continue;
                        }
            ...
            if (mMnbRecoveryRequests[itMnbReplies->first].first < GetTime()) {
                ...
                if (itMnbReplies->second.size() >= MNB_RECOVERY_QUORUM_REQUIRED) {
                    ...
                    LogPrint("masternode", "CMasternodeMan::CheckAndRemove -- reprocessing mnb, masternode=%s\n",
                             itMnbReplies->second[0].pubKeyMasternode.GetID().ToString());
                ...
                LogPrint("masternode", "CMasternodeMan::CheckAndRemove -- removing mnb recovery reply, masternode=%s, size=%d\n",
                         itMnbReplies->second[0].pubKeyMasternode.GetID().ToString(), (int)itMnbReplies->second.size());
    ...
    }
    {
        ...
        std::map<CPubKey, std::map<CNetAddr, int64_t> >::iterator it2 = mWeAskedForMasternodeListEntry.begin();
...
CMasternode* CMasternodeMan::Find(const CPubKey& pubKey)
{
    LOCK(cs);
    for (auto& mnpair : mapMasternodes) {
        if (mnpair.second.pubKeyMasternode == pubKey) {
...
bool CMasternodeMan::Get(const CPubKey& pubKey, CMasternode& masternodeRet)
{
    // Theses mutexes are recursive so double locking by the same thread is safe.
    LOCK(cs);
    auto it = mapMasternodes.find(pubKey);
...
bool CMasternodeMan::GetMasternodeInfo(const CScript& payee, masternode_info_t& mnInfoRet)
{
    LOCK(cs);
    for (auto& mnpair : mapMasternodes) {
        CScript scriptCollateralAddress = GetScriptForDestination(mnpair.second.pubKeyMasternode.GetID());
...
bool CMasternodeMan::Has(const CPubKey& pubKey)
{
    LOCK(cs);
    return mapMasternodes.find(pubKey) != mapMasternodes.end();
}
...
bool CMasternodeMan::GetMasternodeRank(const CPubKey &pubKey, int& nRankRet, int nBlockHeight, int nMinProtocol)
{
    nRankRet = -1;
    ...
    for (auto& scorePair : vecMasternodeScores) {
        nRank++;
        if(scorePair.second->pubKeyMasternode == pubKey) {
            nRankRet = nRank;
            return true;
        }
    }
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
        ...
        LogPrint("masternode", "MNANNOUNCE -- Masternode announce, masternode=%s\n",
                 mnb.pubKeyMasternode.GetID().ToString());
        ...
        CMasternode* pmn = Find(mnp.pubKeyMasternode);
        ...
        AskForMN(pfrom, mnp.pubKeyMasternode, connman);
        ...
        if (pubkey == CPubKey()) { //only should ask for this once
            //local network
        ...
        for (auto& mnpair : mapMasternodes) {
            if (pubkey != CPubKey() && pubkey != mnpair.second.pubKeyMasternode) continue; // asked for specific vin but we are not there yet
            ...
            LogPrint("masternode", "DSEG -- Sending Masternode entry: masternode=%s  addr=%s\n",
                     mnpair.first.GetID().ToString(), mnpair.second.addr.ToString());
        ...
        // smth weird happen - someone asked us for pubkey we have no idea about?
...
void CMasternodeMan::DoFullVerificationStep(CConnman& connman)
{
    ...
    while(it != vecMasternodeRanks.end()) {
        ...
        if(it->second.pubKeyMasternode == activeMasternode.pubKeyMasternode) {
            nMyRank = it->first;
    ...
    while(it != vecMasternodeRanks.end()) {
        if(it->second.IsPoSeVerified() || it->second.IsPoSeBanned()) {
            LogPrint("masternode", "CMasternodeMan::DoFullVerificationStep -- Already %s%s%s masternode %s address %s, skipping...\n",
                        ...
                        it->second.pubKeyMasternode.GetID().ToString(), it->second.addr.ToString());
        ...
        LogPrint("masternode", "CMasternodeMan::DoFullVerificationStep -- Verifying masternode %s rank %d/%d address %s\n",
                 it->second.pubKeyMasternode.GetID().ToString(), it->first, nRanksTotal, it->second.addr.ToString());
...
void CMasternodeMan::CheckSameAddr()
{
    ...
    BOOST_FOREACH(CMasternode* pmn, vBan) {
        LogPrintf("CMasternodeMan::CheckSameAddr -- increasing PoSe ban score for masternode %s\n",
                  pmn->pubKeyMasternode.GetID().ToString());
...
void CMasternodeMan::ProcessVerifyReply(CNode* pnode, CMasternodeVerification& mnv)
{
...

    {
        ...
        for (auto& mnpair : mapMasternodes) {
            if (CAddress(mnpair.second.addr, NODE_NETWORK) == pnode->addr) {
                if (CMessageSigner::VerifyMessage(mnpair.second.pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
                    ...
                    mnv.pubKey1 = mnpair.second.pubKeyMasternode;
                    mnv.pubKey2 = activeMasternode.pubKeyMasternode;
                    std::string strMessage2 = strprintf("%s%d%s%s%s", mnv.addr.ToString(false), mnv.nonce, blockHash.ToString(),
                                                        mnv.pubKey1.GetID().ToString(), mnv.pubKey2.GetID().ToString());
                    // ... and sign it
        ...
        LogPrintf("CMasternodeMan::ProcessVerifyReply -- verified real masternode %s for addr %s\n",
                  prealMasternode->pubKeyMasternode.GetID().ToString(), pnode->addr.ToString());
        ...
        BOOST_FOREACH(CMasternode* pmn, vpMasternodesToBan) {
            ...
            LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- increased PoSe ban score for %s addr %s, new score %d\n",
                     prealMasternode->pubKeyMasternode.GetID().ToString(), pnode->addr.ToString(), pmn->nPoSeBanScore);
        }
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
...

    {
        ...
        CMasternode* pmn1 = Find(mnv.pubKey1);
        if(!pmn1) {
            LogPrintf("CMasternodeMan::ProcessVerifyBroadcast -- can't find masternode1 %s\n",
                      mnv.pubKey1.GetID().ToString());
            return;
        }

        CMasternode* pmn2 = Find(mnv.pubKey2);
        if(!pmn2) {
        ...
        LogPrintf("CMasternodeMan::ProcessVerifyBroadcast -- verified masternode %s for addr %s\n",
                  pmn1->pubKeyMasternode.GetID().ToString(), pnode->addr.ToString());

        // increase ban score for everyone else with the same addr
        int nCount = 0;
        for (auto& mnpair : mapMasternodes) {
            if (mnpair.second.addr != mnv.addr || mnpair.first == mnv.pubKey1) continue;
...
void CMasternodeMan::UpdateMasternodeList(CMasternodeBroadcast mnb, CConnman& connman)
{
    ...
    LogPrintf("CMasternodeMan::UpdateMasternodeList -- masternode=%s  addr=%s\n",
              mnb.pubKeyMasternode.GetID().ToString(), mnb.addr.ToString());

    CMasternode* pmn = Find(mnb.pubKeyMasternode);
    if (pmn == NULL) {
        if (Add(mnb)) {
        ...
        if (fMasterNode && mnb.pubKeyMasternode == activeMasternode.pubKeyMasternode) {
            ...
            if (mnb.nProtocolVersion == PROTOCOL_VERSION) {
                ...
                LogPrintf("CMasternodeMan::CheckMnbAndUpdateMasternodeList -- Got NEW Masternode entry: masternode=%s  sigTime=%lld  addr=%s\n",
                          mnb.pubKeyMasternode.GetID().ToString(), mnb.sigTime, mnb.addr.ToString());
...
void CMasternodeMan::UpdateLastPaid(const CBlockIndex* pindex)
{
    ...
    if(!masternodeSync.IsWinnersListSynced() || mapMasternodes.empty()) return;
...
{
    LogPrintf("CMasternodeMan::CheckMasternode -- size of masternodes map: %d\n", mapMasternodes.size());
    LOCK(cs);
    for (auto& mnpair : mapMasternodes) {
        if (mnpair.second.pubKeyMasternode == pubKeyMasternode) {
...
bool CMasternodeMan::IsMasternodePingedWithin(const CPubKey& pubKey, int nSeconds, int64_t nTimeToCheckAt)
{
    LOCK(cs);
    CMasternode* pmn = Find(pubKey);
...
void CMasternodeMan::SetMasternodeLastPing(const CPubKey& pubKey, const CMasternodePing& mnp)
{
    LOCK(cs);
    CMasternode* pmn = Find(pubKey);
    if(!pmn) {
