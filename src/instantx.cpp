...
{
    CAmount nTransactionFee = sporkManager.GetSporkValue(SPORK_4_INSTANTSEND_TRANSACTION_FEE);
    return std::max(nTransactionFee, CAmount(std::ceil(vin.size() / 10.0) * nTransactionFee));
}
...
{
    return mapMasternodeVotes.count(mnPubKey);
}
...
bool CTxLockCandidate::HasMasternodeVoted(const COutPoint& outpointIn, const CPubKey& mnPubKey)
{
    ...
    return it !=mapOutPointLocks.end() && it->second.HasMasternodeVoted(mnPubKey);
}
