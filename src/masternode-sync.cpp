    ...
    BOOST_FOREACH(CNode* pnode, vNodesCopy)
    {
        ...
        if(Params().NetworkIDString() == CBaseChainParams::REGTEST)
        {
            if(nRequestedMasternodeAttempt <= 2) {
                ...
                connman.PushMessageWithVersion(pnode, INIT_PROTO_VERSION, NetMsgType::GETMNLIST); //get mnlist
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            ...
            if(!netfulfilledman.HasFulfilledRequest(pnode->addr, "mnlist-sync")) {
                netfulfilledman.AddFulfilledRequest(pnode->addr, "mnlist-sync");
                connman.PushMessageWithVersion(pnode, INIT_PROTO_VERSION, NetMsgType::GETMNLIST);
                LogPrintf("CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- requesting mnlist from peer %d\n", nTick, nRequestedMasternodeAssets, pnode->id);
            }
...
void CMasternodeSync::UpdatedBlockTip(const CBlockIndex *pindexNew, bool fInitialDownload, CConnman& connman)
{
    ...
    if (fReachedBestHeader && !fReachedBestHeaderNew) {
        /* Switching from true to false means that we previously stuck syncing headers
         * for some reason, probably initial timeout was not enough, because
         * there is no way we can update tip not having best header.
         */
        LogPrint("mnsync", "CMasternodeSync::UpdatedBlockTip -- still not having best header\n");
        Reset();
