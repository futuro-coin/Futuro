...
#include "config/futurocoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("futurocoin-shutoff");
    ...
    CFlatDB<CMasternodeListManager> flatdb3("mnsynclist.dat", "magicMasternodeListCache");
    flatdb3.Dump(masternodeListManager);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "futurocoin (or specifically: instantsend, keepass, masternode, mnpayments, mnsync, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("futurocoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that FuturoCoin Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("FuturoCoin Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize FuturoCoin Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FuturoCoin Core is shutting down."));
...
#endif
    // Make sure only a single FuturoCoin Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FuturoCoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FuturoCoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (mapArgs.count("-mnlistmanagerkey")) // masternode list manager priv key
    {
        if (!masternodeListManager.SetPrivKey(GetArg("-mnlistmanagerkey", "")))
            return InitError(_("Unable to sign masternode list message, wrong key?"));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FuturoCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart FuturoCoin Core to complete") << "\n";
    ...
    strDBName = "mnsynclist.dat";
    uiInterface.InitMessage(_("Loading masternode sync list cache..."));
    CFlatDB<CMasternodeListManager> flatdb3(strDBName, "magicMasternodeListCache");
    if (!flatdb3.Load(masternodeListManager)) {
        return InitError(_("Failed to load masternode sync list cache from") + "\n" + (pathDB / strDBName).string());
    }
    ...
    // ********************************************************* Step 11c: update block tip in FuturoCoin modules
    ...
    // ********************************************************* Step 11d: start futuro-mnsync thread
