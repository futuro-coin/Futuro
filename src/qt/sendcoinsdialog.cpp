...
void SendCoinsDialog::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel) {
        connect(clientModel, SIGNAL(numBlocksChanged(int,QDateTime,double,bool)), this, SLOT(updateFeeMinimizedLabel()));
        connect(clientModel, SIGNAL(instantSendFeeChanged()), this, SLOT(updateFeeMinimizedLabel()));
    }
}
...
void SendCoinsDialog::clear()
{
    ...
    addEntry();
    ui->addButton->setEnabled(ui->entries->count() < MAX_OUTPUTS_ALLOWED - 1);
    updateTabsAndLabels();
}
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    ui->labelFeeMinimized->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
                                                                CTxLockRequest().GetMinFee()) + "/10 inputs");
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid FuturoCoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
