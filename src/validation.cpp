...
#if defined(NDEBUG)
# error "FuturoCoin Core cannot be compiled without assertions."
#endif
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (!tx.IsCoinBase() && tx.vout.size() > MAX_OUTPUTS_ALLOWED)
        return state.DoS(10, false, REJECT_INVALID, "bad-txns-vout-number-allowed-exceed");
    // Size limits
...
{
    /* FuturoCoin uses static reward mechanism. Each block which is mined every
     * minute will provide 13.31811263 Futuros. Aim is to provide a reward for
     * at the most 10 years.
     * The formula 2222222/(((Difficulty+2600)/9)^2) used currently in Dash
     * is not applicable to Futuro at the moment. There is also no need
     * to distinguish between early ages, CPU mining era and GPU/ASIC mining era.
     **/

    double nSubsidyBase = 0;

    /* The final block number is calculated as follow:
     * 10(years) * 365(days)* 24(hours)* 60()minutes. */
    int finalBlockNumber = 5256000;

    /* The first mined block should be awarded a 30 000 000 futuros)*/
    if (nPrevHeight < 1) {
        nSubsidyBase = 30000000;
    }
    else if (nPrevHeight <= finalBlockNumber) {
        nSubsidyBase = 13.31811263;
    }
...
{
    /* In Futuro reward is divided equally between the Miner
     * and the Masternode, except for the first block */
...
void ThreadScriptCheck() {
    RenameThread("futurocoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // FUTURO : MODIFIED TO CHECK MASTERNODE PAYMENTS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(FUTURO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(FUTURO): couldn't find masternode payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // FUTURO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(FUTURO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(FUTURO): spork is off, skipping transaction locking checks\n");
    }
